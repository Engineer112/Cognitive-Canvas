...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const savedDock =
    localStorage.getItem("cognitive_toolbar_dock") || "dock-top";
  ...
  document.addEventListener("pointerup", (e) => {
    if (isDraggingToolbar) {
      ...
      let newDock =
        currentHoveredZone ||
        localStorage.getItem("cognitive_toolbar_dock") ||
        "dock-top";
...
window.renderPathUI = function () {
  ...
  window.storyPaths.forEach((p, idx) => {
    ...
    opt.value = p.id;
    opt.innerText =
      p.name + (p.id === window.compiledPathId ? " (Compile)" : "");
...
window.treeViewExpanded = window.treeViewExpanded || new Set();

window.renderTreeView = function () {
  const list = document.getElementById("tree-view-content");
  if (!list) return;
  list.innerHTML = "";

  const included = window.nodeSequence || [];
  const allIds = Array.from(window.nodesMap.keys());
  const excluded = allIds.filter((id) => !included.includes(id));

  const childMap = new Map();
  allIds.forEach((id) => childMap.set(id, []));

  const remainingExcluded = new Set(excluded);

  if (window.edges) {
    window.edges.forEach((e) => {
      if (remainingExcluded.has(e.target)) {
        childMap.get(e.source).push(e.target);
        remainingExcluded.delete(e.target);
      }
    });
  }
  ...
  function renderNode(
    ...
    if (!titleText) {
      ...
      if (editor && editor.innerText)
        titleText = editor.innerText.trim().substring(0, 20) + "...";
      else titleText = isIncluded ? `Thought ${index + 1}` : "Detached Thought";
    }

    const wrapper = document.createElement("div");
    wrapper.id = "tree-node-wrapper-" + nodeId;

    const nodeEl = document.createElement("div");
    nodeEl.className = isIncluded ? "tree-node level-1" : "tree-node level-2";

    if (window.selectedNodes && window.selectedNodes.has(nodeId)) {
      nodeEl.classList.add("tree-node-selected");
    }

    if (!isIncluded && level > 1) {
    ...
    if (isDetachedRoot) {
      ...
      nodeEl.style.paddingLeft = "6px";
      nodeEl.style.color = "var(--text-color)";
      nodeEl.style.opacity = "0.6";
    }

    const children = childMap.get(nodeId) || [];
    const hasChildren = children.length > 0;
    const isExpanded = window.treeViewExpanded.has(nodeId);

    let toggleHtml = "";
    if (hasChildren) {
      toggleHtml = `<span class="toggle-btn" style="cursor:pointer; display:inline-block; width:16px; text-align:center; flex-shrink:0;">${isExpanded ? "▼" : "▶"}</span>`;
    } else if (!isIncluded && !isDetachedRoot) {
      toggleHtml = `<span style="opacity:0.5; width:16px; display:inline-block; text-align:center; flex-shrink:0;">↳</span>`;
    }

    let indicatorHtml = "";
    if (isIncluded) {
      indicatorHtml = `<span style="background:var(--focus-ring); color:white; border-radius:50%; width:22px; height:22px; display:inline-flex; align-items:center; justify-content:center; font-size:0.75rem; flex-shrink:0; margin-left:4px; margin-right:4px;">${index + 1}</span>`;
    } else if (isDetachedRoot) {
      indicatorHtml = `<span style="background:gray; color:white; border-radius:50%; width:22px; height:22px; display:inline-flex; align-items:center; justify-content:center; font-size:0.75rem; flex-shrink:0; margin-left:4px; margin-right:4px;">-</span>`;
    }

    let actionsHtml = `
                    <div class="tree-node-actions">
                        <button class="tree-action-btn tree-add-btn" title="Add subnode">+</button>
                        <button class="tree-action-btn tree-del-btn" title="Delete node">✕</button>
                    </div>
                `;

    nodeEl.innerHTML = `${toggleHtml}${indicatorHtml} <span style="white-space:nowrap; overflow:hidden; text-overflow:ellipsis; flex:1; cursor:pointer; margin-left:4px;" class="node-title">${titleText}</span>${actionsHtml}`;

    const addBtn = nodeEl.querySelector(".tree-add-btn");
    if (addBtn) {
      const addHandler = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (window.addTreeNode) window.addTreeNode(nodeId, e);
      };
      addBtn.onclick = addHandler;
      addBtn.onpointerdown = addHandler;
    }

    const delBtn = nodeEl.querySelector(".tree-del-btn");
    if (delBtn) {
      const delHandler = (e) => {
        e.preventDefault();
        e.stopPropagation();
        if (window.deleteTreeNode) window.deleteTreeNode(nodeId, e);
      };
      delBtn.onclick = delHandler;
      delBtn.onpointerdown = delHandler;
    }
    ...
    if (hasChildren) {
      ...
      if (toggleBtn) {
        toggleBtn.onclick = (e) => {
          e.stopPropagation();
          if (isExpanded) {
            window.treeViewExpanded.delete(nodeId);
          } else {
            window.treeViewExpanded.add(nodeId);
          }
          window.renderTreeView();
        };
      }
    ...
    nodeEl.onclick = (e) => {
      e.stopPropagation();
      if (window.panToNode) window.panToNode(nodeId);
    ...
    if (hasChildren && isExpanded) {
      ...
      children.forEach((childId) => {
        const childEl = renderNode(childId, level + 1, false);
  ...
  included.forEach((id, index) => {
    const el = renderNode(id, 0, true, index);
    if (el) list.appendChild(el);
  });

  if (remainingExcluded.size > 0) {
    remainingExcluded.forEach((childId) => {
      const el = renderNode(childId, 0, false, null, true);
      if (el) list.appendChild(el);
    });
  }
};

window.addTreeNode = function (parentId, evt) {
  ...
  if (parentId) {
    const parentNode = window.nodesMap.get(parentId);
    if (parentNode) {
      x =
        parseFloat(parentNode.style.left || 0) +
        parseFloat(parentNode.style.width || 300) +
        60;
      y = parseFloat(parentNode.style.top || 0);

      if (window.edges) {
        const childrenIds = window.edges
          .filter((e) => e.source === parentId)
          .map((e) => e.target);
        if (childrenIds.length > 0) {
          let maxY = y - 120;
          childrenIds.forEach((id) => {
            const childNode = window.nodesMap.get(id);
            if (childNode) {
    ...
    x = (x - (window.panX || 0)) / (window.scale || 1);
    y = (y - (window.panY || 0)) / (window.scale || 1);

    // Offset top-Level nodes if there are already some
    if (window.nodeSequence && window.nodeSequence.length > 0) {
      let maxY = y - 160;
      window.nodeSequence.forEach((id) => {
        const rootNode = window.nodesMap.get(id);
        if (rootNode) {
          const rootX = parseFloat(rootNode.style.left || 0);
          const rootY = parseFloat(rootNode.style.top || 0);
          // Only stagger if they are somewhat near the center where we intend to spawn
          if (Math.abs(rootX - x) < 300 && rootY > maxY) {
            maxY = rootY;
          }
        }
      });
      y = Math.max(y, maxY + 160);
    }
  }

  const newId = "node_" + Date.now() + Math.random().toString(36).substr(2, 5);
  if (window.createNode) window.createNode(x, y, "", newId);

  if (parentId) {
    if (!window.edges) window.edges = [];
    window.edges.push({
      id: "edge_" + Date.now(),
      source: parentId,
      target: newId,
    ...
    // Remove from nodeSequence since it's a subnode
    if (window.nodeSequence) {
      window.nodeSequence = window.nodeSequence.filter((id) => id !== newId);
    ...
    window.treeViewExpanded.add(parentId);
  }

  if (window.saveState) window.saveState();
  if (window.renderTreeView) window.renderTreeView();
  if (window.selectNode) window.selectNode(newId);
};

window.deleteTreeNode = function (nodeId, evt) {
  if (evt) evt.stopPropagation();
  if (window.deleteNode) window.deleteNode(nodeId, true);
  if (window.renderTreeView) window.renderTreeView();
};
...
window.forkPath = function () {
  ...
  let newPath = {
    id: "p_" + Date.now(),
    name: "Path " + letter,
    sequence: [...window.nodeSequence],
    edges: JSON.parse(JSON.stringify(window.edges)),
  };
...
window.addEdge = function (sourceId, targetId) {
  if (window.edges.some((e) => e.source === sourceId && e.target === targetId))
  ...
  if (window.nodeSequence && window.nodeSequence.length > 0) {
    if (
      !window.nodeSequence.includes(sourceId) ||
      !window.nodeSequence.includes(targetId)
    ) {
      compileFlag = false;
    }
  }
  ...
  window.edges.push({
    id: "edge_" + Date.now() + Math.floor(Math.random() * 1000),
    source: sourceId,
    target: targetId,
    compile: compileFlag,
  });
...
window.toggleEdgeCompile = function (edgeId) {
  ...
  if (edge) {
    if (!edge.compile) {
      // Backup states
      const backups = window.edges.map((e) => ({
        id: e.id,
        compile: e.compile,
      }));
      ...
      if (roots > 1) {
        ...
        window.showToast &&
          window.showToast(
            "Cannot include tether. It must be connected to the main active path.",
          );
        return;
      }
    } else {
...
window.updateGraph = function () {
  ...
  let roots = Array.from(window.nodesMap.keys()).filter(
    (id) => inDegreeAll.get(id) === 0,
  );
  ...
  roots.sort((a, b) => {
    let na = window.nodesMap.get(a),
      nb = window.nodesMap.get(b);
    if (!na || !nb) return 0;
    return (
      parseFloat(na.style.top) - parseFloat(nb.style.top) ||
      parseFloat(na.style.left) - parseFloat(nb.style.left)
    );
  });
  ...
  while (queue.length > 0) {
    ...
    outs.sort((ea, eb) => {
      let na = window.nodesMap.get(ea.target),
        nb = window.nodesMap.get(eb.target);
      if (!na || !nb) return 0;
      return (
        parseFloat(na.style.top) - parseFloat(nb.style.top) ||
        parseFloat(na.style.left) - parseFloat(nb.style.left)
...
window.clearSelection = function () {
  ...
  if (window.clearPathHighlights) window.clearPathHighlights();
};

window.highlightNodePaths = function (nodeId) {
  const layer = document.getElementById("tether-layer");
  if (!layer || !nodeId) return;
  layer.classList.add("path-highlight-mode");

  layer.querySelectorAll(".tether-group").forEach((group) => {
    const src = group.getAttribute("data-source-id");
    const tgt = group.getAttribute("data-target-id");
...
window.clearPathHighlights = function () {
  ...
  layer.classList.remove("path-highlight-mode");
  layer.querySelectorAll(".tether-group").forEach((group) => {
    group.classList.remove("highlighted");
  });
};
...
window.updateMarqueeSelection = function (e) {
  ...
  window.nodesMap.forEach((node, id) => {
    ...
    const overlap = !(
      nodeRect.right < marqueeRect.left ||
...
window.selectNode = function (id, toggle = false) {
  ...
  if (panel && panel.classList.contains("visible")) {
    window.renderTreeView();
    if (selectedNodes.has(id)) {
      // Find the tree node and scroll to it
      const treeNode = document.getElementById("tree-node-wrapper-" + id);
      if (treeNode) {
...
window.saveCurrentTabState = function () {
  ...
  if (tab) {
    let activeP = window.storyPaths.find(
      (p) => p.id === window.activeEditPathId,
    );
    if (activeP) activeP.sequence = [...window.nodeSequence];
    tab.nodesMap = window.nodesMap;
    tab.nodeSequence = window.nodeSequence;
    tab.edges = window.edges ? JSON.parse(JSON.stringify(window.edges)) : [];
...
window.renderTabBar = function () {
  ...
  window.tabs.forEach((tab) => {
    ...
    tabEl.onkeydown = (e) => {
      if (e.key === "Enter" || e.key === " ") window.switchTab(tab.id);
    };
...
window.switchTab = function (tabId) {
  ...
  window.nodeSequence = tab.nodeSequence ? [...tab.nodeSequence] : [];
  window.edges = tab.edges ? JSON.parse(JSON.stringify(tab.edges)) : [];
  if (tab.storyPaths) {
...
window.addNewTab = function () {
  ...
  window.createNode(
    ...
    `Double-tap the space to add a thought.
...
window.restoreState = function (state) {
  ...
  if (state.sequence) {
    window.nodeSequence = [...state.sequence];
    window.edges = state.edges ? JSON.parse(JSON.stringify(state.edges)) : [];
  }
  if (state.storyPaths) {
...
let savedShortcuts =
  JSON.parse(localStorage.getItem("cognitive_shortcuts")) || {};
let userShortcuts = {
...
document.addEventListener("keydown", (e) => {
  if (isRecordingShortcut) {
    e.preventDefault();
    if (
      ["control", "shift", "alt", "meta", "os"].includes(
        (e.key || "").toLowerCase(),
      )
    )
      return;
  ...
  if (!isInput) {
    // Map Delete or Backspace to delete the currently selected node(s)
    ...
    // Arrow keys for panning or selection shifting
    if (e.key === "Home") {
      e.preventDefault();
      window.goToRootNode && window.goToRootNode();
      return;
    }
    ...
    if (["ArrowUp", "ArrowDown", "ArrowLeft", "ArrowRight"].includes(e.key)) {
      ...
      if (window.selectedNodes && window.selectedNodes.size === 1) {
        const currentId = Array.from(window.selectedNodes)[0];
        const currentNode = window.nodesMap.get(currentId);
        if (!currentNode) return;
        const currentX = parseFloat(currentNode.style.left);
        const currentY = parseFloat(currentNode.style.top);
        ...
        candidates.forEach((id) => {
          ...
          let isValid = false;
          if (e.key === "ArrowUp" && dy < -10 && Math.abs(dy) > Math.abs(dx))
            isValid = true;
          if (e.key === "ArrowDown" && dy > 10 && Math.abs(dy) > Math.abs(dx))
            isValid = true;
          if (e.key === "ArrowLeft" && dx < -10 && Math.abs(dx) > Math.abs(dy))
            isValid = true;
          if (e.key === "ArrowRight" && dx > 10 && Math.abs(dx) > Math.abs(dy))
            isValid = true;
        ...
        if (bestMatch) {
          window.clearSelection();
          window.selectNode(bestMatch);
          window.panToNode(bestMatch);
        }
      } else if (!window.selectedNodes || window.selectedNodes.size === 0) {
        const panAmount = 50;
        if (e.key === "ArrowUp") window.panY += panAmount;
        if (e.key === "ArrowDown") window.panY -= panAmount;
        if (e.key === "ArrowLeft") window.panX += panAmount;
        if (e.key === "ArrowRight") window.panX -= panAmount;
        window.updateCanvasTransform();
      }
      return;
    }

    // Map Cmd/Ctrl + Z to undo, Cmd/Ctrl + Shift + Z or Cmd/Ctrl + Y to redo
  ...
  for (let [action, combo] of Object.entries(userShortcuts)) {
    if (
      combo.key === pressedCombo.key &&
      combo.ctrl === pressedCombo.ctrl &&
      combo.alt === pressedCombo.alt &&
      combo.shift === pressedCombo.shift
    ) {
      ...
      switch (action) {
        ...
        case "duplicate": {
          ...
          selectedNodes.forEach((id) => {
            ...
            if (n) {
              ...
              const newId =
                "node_" + Date.now() + Math.floor(Math.random() * 1000);
              window.createNode(
                nx,
                ny,
                text,
        ...
        case "readAloud":
          if (document.getElementById("compile-modal").style.display !== "flex")
            window.compileStory();
        ...
        case "dictate": {
          ...
          if (actEd && actEd.classList.contains("editor")) {
            ...
            window.showToast("Focus inside a thought bubble to dictate.");
          }
          break;
        }
        case "newNode":
        ...
        case "settings":
          ...
          break;
        case "color1":
          window.applyColorToActiveNode("default");
          break;
        case "color2":
          window.applyColorToActiveNode("blue");
          break;
        case "color3":
          window.applyColorToActiveNode("green");
          break;
        case "color4":
          window.applyColorToActiveNode("purple");
          break;
        case "color5":
          window.applyColorToActiveNode("yellow");
          break;
        case "color6":
          window.applyColorToActiveNode("red");
          break;
      }
      return;
    }
...
window.applySettings = function () {
  ...
  root.style.setProperty(
    "--font-family",
    document.getElementById("set-font").value,
  );
  ...
  if (spacing === "normal") {
    ...
    root.style.setProperty("--letter-spacing", "0.06em");
  } else {
  ...
  if (size === "small") root.style.setProperty("--editor-font-size", "1rem");
    ...
    root.style.setProperty("--editor-font-size", "1.15rem");
  else if (size === "large")
    root.style.setProperty("--editor-font-size", "1.35rem");
  else if (size === "xlarge")
    root.style.setProperty("--editor-font-size", "1.6rem");
};
...
let activeDictationEditor = null,
  activeMicBtn = null;
if (recognizer) {
  recognizer.onresult = (e) => {
    if (activeDictationEditor) {
      ...
      if (newText) {
        ...
        activeDictationEditor.innerText +=
          (currentText &&
...
const tutorialChunks = [
  {
    ...
    successDesc:
      "You've got the basics down. Want to learn how to connect your thoughts?",
    steps: [
      {
  ...
  },
  {
    ...
    successDesc:
      "Awesome job building a flow. Next up: Adding colors and media!",
    steps: [
      {
        title: "Link Thoughts 🔗",
        desc: "Drag the numbered badge onto another thought, or onto empty space to spawn a new thought linked to it.",
        waitFor: "link_node",
        cssId: 3,
        action: () => {
      ...
      {
        title: "Insert Between ✂️",
        desc: "Drag a single thought directly onto a connection line to snap it into the middle, or right-click the line to delete it!",
        waitFor: "insert_between",
        cssId: 4,
        action: () => {
  ...
  },
  {
    ...
    successDesc:
      "Your canvas is coming to life. Ready to learn how to organize big projects?",
    steps: [
      {
        title: "Color Coding 🎨",
...
window.dismissWelcome = function () {
  ...
  window.showToast(
    "Tutorial skipped. You can always restart it from the Settings menu!",
    false,
  );
};
...
window.nextTutStep = function () {
  ...
  if (currentTutStepIndex < chunk.steps.length - 1) {
    ...
    if (currentChunkIndex < tutorialChunks.length - 1) {
      // Show Interstitial
      for (let i = 0; i <= 15; i++)
        document.body.classList.remove(`tut-step-${i}`);
      document.getElementById("tut-normal-view").style.display = "none";
      document.getElementById("tut-chunk-view").style.display = "flex";

      document.getElementById("tut-chunk-finish-title").innerText =
        chunk.successTitle;
      document.getElementById("tut-chunk-finish-desc").innerText =
        chunk.successDesc;
      document.getElementById("tut-start-chunk-btn").innerText =
        `Start: ${tutorialChunks[currentChunkIndex + 1].name.split(".")[1].trim()} ➔`;
      window.playSound("drop");
    } else {
...
window.updateTutUI = function () {
  ...
  document.getElementById("tut-progress").innerText =
    `${currentTutStepIndex + 1}/${chunk.steps.length}`;
...
async function initCognitiveApp() {
  try {
    ...
    document.getElementById("loading-screen").innerText =
      "An error occurred during boot: " + error.message;
  }
}
...
window.deleteNode = function (nodeId, save = true) {
  ...
  if (window.edges) {
    window.edges = window.edges.filter(
...
window.createNode = function (
  ...
  titleInput.addEventListener(
    "click",
    () => window.triggerTutAction && window.triggerTutAction("name_node"),
  );
  ...
  gripBar.className = "node-grip";
  gripBar.addEventListener("pointerdown", handleNodeDragStart);

  dragHandle.addEventListener("pointerdown", handleNodeDragStart);
  ...
  node.addEventListener("pointerenter", () => {
    if (window.highlightNodePaths) window.highlightNodePaths(nodeId);
  });
  node.addEventListener("pointerleave", () => {
    if (window.clearPathHighlights) window.clearPathHighlights();
  });

  node.appendChild(gripBar);
...
window.showTetherMenu = function (e, edgeId) {
  ...
  menu.innerHTML =
    "<button onclick=\"window.toggleEdgeCompile('" +
    edgeId +
    '\'); this.parentNode.remove();" class="text-style-btn">' +
    (edge.compile ? "❌ Exclude from Compile" : "✅ Include in Compile") +
    "</button>" +
    '<div style="height: 1px; background: var(--surface-border); margin: 4px 0;"></div>' +
    "<button onclick=\"window.deleteEdge('" +
    edgeId +
    '\'); this.parentNode.remove();" class="text-style-btn" style="color: var(--error-border)">🗑️ Delete Tether</button>';
...
const getBoundPoint = (left, top, width, height, cx, cy, tx, ty) => {
  ...
  if (Math.abs(dx) < 0.1 && Math.abs(dy) < 0.1)
    return { x: cx, y: cy + height / 2 + 5, side: "bottom" };
...
window.updateTethers = function () {
  ...
  let svgStr =
    ...
    '<marker id="arrow-gray" viewBox="0 0 10 10" refX="8" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><path d="M 0 0 L 10 5 L 0 10 z" fill="#9ca3af" /></marker>' +
    "</defs>";
  ...
  window.edges.forEach((edge) => {
    ...
    let p1 = getBoundPoint(
      parseFloat(n1.style.left),
      parseFloat(n1.style.top),
      n1.offsetWidth,
      n1.offsetHeight,
      cx1,
      cy1,
    ...
    let p2 = getBoundPoint(
      parseFloat(n2.style.left),
      parseFloat(n2.style.top),
      n2.offsetWidth,
      n2.offsetHeight,
      cx2,
      cy2,
    ...
    let cp1x =
      p1.x +
      (p1.side === "right" ? cpOffset : p1.side === "left" ? -cpOffset : 0);
    let cp1y =
      p1.y +
      (p1.side === "bottom" ? cpOffset : p1.side === "top" ? -cpOffset : 0);

    let cp2x =
      p2.x +
      (p2.side === "right" ? cpOffset : p2.side === "left" ? -cpOffset : 0);
    let cp2y =
      p2.y +
      (p2.side === "bottom" ? cpOffset : p2.side === "top" ? -cpOffset : 0);

    svgStr +=
      '<g class="tether-group" data-edge-id="' +
      edge.id +
      '" data-source-id="' +
      edge.source +
      '" data-target-id="' +
      edge.target +
      '" role="button" tabindex="0" oncontextmenu="window.showTetherMenu(event, \'' +
      edge.id +
      "')\" onclick=\"window.panToNode('" +
      edge.target +
      "')\" onkeydown=\"if(event.key === 'Enter' || event.key === ' ') { event.preventDefault(); window.panToNode('" +
      edge.target +
      "'); }\">" +
      '<path d="M' +
      p1.x +
...
document.addEventListener("pointerup", (e) => {
  if (isDraggingNode) {
    ...
    if (
      ...
      if (targetTether) {
        ...
        if (
          edgeToReplace &&
          draggedNode.id !== edgeToReplace.source &&
          draggedNode.id !== edgeToReplace.target
        ) {
              ...
              window.edges = window.edges.filter(
                (e) =>
                  !(
                    ...
                    e.target === edgeToReplace.target
                  ),
              );
            window.addEdge(edgeToReplace.source, edgeToReplace.target);
          } else {
            window.triggerTutAction &&
              window.triggerTutAction("insert_between");
          }
          window.updateGraph && window.updateGraph();
          window.updateTethers && window.updateTethers();
        }
  ...
  if (isLinking) {
    ...
    document
      .querySelectorAll(".seq-badge")
      .forEach((b) => b.classList.remove("linking"));
